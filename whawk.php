...
function update()
    {
        echo "\n\e[97m\e[1m[+] WHITE HAWK UPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln";
        system("git fetch origin && git reset --hard origin/master && git clean -f -d");
        echo $bold . $fgreen . "[i] Job finished successfully! Please Restart WHITE HAWK \n" . $cln;
        exit;
    }
...
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] WHITE HAWK FiX MENU [+]\n\n$cln";
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want WHITE HAWK To Detect CMS Of The Sites? [Y/N]");
                ...
                if ($wp_version != "")
                  {
                    ...
                    if (strpos($vuln_json, "The page you were looking for doesn't exist (404)") !== false)
                      {
                        echo $red . "[!] Seems like the version detection messed up preety bad! Please report here: WhiteDevil-404\n";
                      }
                    else
                      {
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] WHITE HAWK could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ WHITE HAWK Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("https://raw.githubusercontent.com/WhiteDevil-sys/White-Hawk/main/version.txt");
            ...
            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of WHITE HAWK. \n\n" . $cln;
                goto scanlist;
              }
            else
              {
